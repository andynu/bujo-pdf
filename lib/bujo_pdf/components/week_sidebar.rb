  ...
  module Components
    ...
    class WeekSidebar < SidebarBase
      ...
      def draw_week_entry(week, row)
        ...
        is_current = current_week?(week)

        # Use base class helper for background
        draw_item_background(week_box, is_current)
      ...
      def draw_current_week(week_box, month_abbrev, week_text)
        ...
        text_x = week_box[:x] + grid.width(sidebar_padding_boxes) - 5
        text_width = week_box[:width] - grid.width(sidebar_padding_boxes * 2)
        ...
        with_fill_color(color_text_gray) do
          if month_abbrev
            ...
            pdf.formatted_text_box [
              { text: "#{month_abbrev} ", styles: [:bold], size: sidebar_font_size, color: color_text_gray },
              { text: week_text, size: sidebar_font_size, color: color_text_gray }
            ],
                          at: [text_x, week_box[:y]],
                          width: text_width,
                          height: week_box[:height],
