...
module BujoPdf
  module Components
    ...
    #   GridRuler.new(@pdf, @grid_system).render
    #
    #   # Or render only columns or rows:
    #   GridRuler.new(@pdf, @grid_system).render_columns
    #   GridRuler.new(@pdf, @grid_system).render_rows
    #
    class GridRuler
      # Default styling
      FONT_SIZE = 6
      ...
      def initialize(pdf, grid_system, font_size: FONT_SIZE, color: COLOR)
        @pdf = pdf
        @grid = grid_system
        @font_size = font_size
        @color = color
